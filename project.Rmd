...
nrow(hyp1)
#558
sapply(hyp, function(x) if (is.factor(x)) length(levels(x)) else NA)
sapply(droplevels(hyp1), function(x) if (is.factor(x)) length(levels(x)) else NA)
```
...
So there seems to be no major difference between the two datasets (hyp, with levels altered and all_dat) when I omit the NA values
...
slr <- lm(BPQ020 ~ ., data = hyp1)
slr <- summary(lm(BPQ020 ~ SMQ020, data = hyp1))
```


```{r}
#Logistic regression
hyp1 <- hyp1 %>% mutate(BPQ020=ifelse(BPQ020=="Yes",1,0))
...
log_reg <- glm(BPQ020 ~ ., data = hyp1, family = binomial())
summary(log_reg)
