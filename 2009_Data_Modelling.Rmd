...
set.seed(1)
nn <- nnet(Hypertension ~ ., data=data2, size=10)
nn
head(data2$Hypertension)
pred <- predict(nn, data2, type="class")
head(pred)
table(data2$Hypertension, pred)
mean( data2$Hypertension != pred)
...
data = data.frame(predicted = predict(log_reg1), actual = data2$Hypertension)
