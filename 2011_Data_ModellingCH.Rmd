...
library(car)
Anova(log_reg1)  # ch: this shows which terms are significant. 
```
...
# It is not lack of fit, it is saying that age and bmi could do with quadratic term. CH
...
```{r}
ch1 <- glm(Hypertension ~ Gender + Age + Race + Smoking + Diabetes + 
...
anova(ch1, ch2, test="LRT")  # keep ch2

ch3 <- glm(Hypertension ~ ( Age + I(Age^2)+ Race + Smoking + Diabetes + 
                     Insulin + KidneyF + BMI + I(BMI^2)+ PhysActivity + Alcohol + Salt +
                     Potassium)*Gender, data=data_11a, family = binomial())
anova(ch2, ch3, test="LRT")
Anova(ch3)  # looks like only need a few of interations

ch4 <- glm(Hypertension ~ ( Age + I(Age^2))*Gender + Race + Smoking + Diabetes + 
                     Insulin + KidneyF + (BMI + I(BMI^2))*Gender + PhysActivity + Alcohol + Salt +
                     Potassium, data=data_11a, family = binomial())

anova(ch4, ch3, test="LRT")  # just keep a few interactions

Anova(ch4)

ch5 <- glm(Hypertension ~ ( Age + I(Age^2))*Gender + Race + Smoking + Diabetes + 
                      KidneyF + (BMI + I(BMI^2))*Gender 
              , data=data_11a, family = binomial())

anova(ch5,ch4, test="LRT")

ch6 <- gam(Hypertension ~ (bs(Age,df=5))*Gender + Race + Smoking + Diabetes + 
                      KidneyF + (BMI + I(BMI^2))*Gender 
              , data=data_11a, family = binomial())

anova(ch5,ch6, test="LRT")  # don't need bs term

```
CH: not saying this is the definitive model choice here.
