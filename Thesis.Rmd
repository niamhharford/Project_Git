...
f5 <- glm(Hypertension ~ (Age+I(Age^2) + BMI+I(BMI^2))*Gender+Race + Smoking + Diabetes + (Diabetes*Age) + KidneyF + PhysActivity + Alcohol + Salt + Potassium, data=data_11a, family = binomial())
...
ex_f5 <- DALEX::explain(f5, data = data_11a, y = hypn, type = "classification")
...
ex_f5_test <- DALEX::explain(f5, data = Test, y = hypn1, type = "classification")
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.cap="Partial dependence plot on the training data for the logistic regression response 2 for age, bmi and potassium, for males and females separately.\\label{fig:bla}",fig.show='hold'}
plot(model_profile(ex_f5,groups="Gender")) + ggtitle("Response for Logistic Regression") + theme(plot.title = element_text(size = 10))
```
...
Figure \ref{fig:rf_var} displays the mean decrease in accuracy and the Gini index totaled over splits involving each predictor. The mean decrease in accuracy is a measure of the model performance without each variable. The higher the variable is on this plot the more important the variable is. If the variable is removed, it was cause a decrease in model accuracy. Age is estimated to be the most important variable in the MeanDecreaseAccuracy plot in the random forest model, if it is removed there is a clear loss of accuracy in the model. Following the potential removal of the Age variable, there appears to be very small amounts of accuracy lost. This highlights the significant of Age. The mean decrease in Gini plot also exhibits the variable importance. The MeanDecreaseGini plot provides evidence that Age, BMI and Potassium, all continuous variables are the most importance variables in this data set. 



MeanDecreaseAccuracy - The first measure is computed from the initial out of bag data (OOB data for each tree in the RF only a sample of the data is used, so for the data that is not used you can get a prediction from the tree, if you do the same after permuting the predictor variables, so you're not removing the variable age you're removing the association with age and hypertension, its telling you how much a shuffled age contributes to the accuracy).

MeanDecreaseGini - Everytime you split on age on the tree, it calculates how much that is contributing to the Gini index and then it averages that over all the trees. The splits on age are giving you the most reduction on the Gini index.
...
plot(model_performance(ex_rf_test),model_performance(ex_log_test),model_performance(ex_nn_test),model_performance(ex_f5_test), geom = "roc")+ theme(plot.title = element_text(size = 10), plot.tag = element_text(size = 5)) + geom_line(lwd = 0.5) 
```

Table \ref{tbl:data1} displays the error rate for all models. The random forest model produced the lowest test error rate at `r`  25.82%, with the logistic regression at 27.01%. The random forest model is the better fit model.
...
f5_auc <- model_performance(ex_f5)$measures$auc
f5_acc <- model_performance(ex_f5)$measures$accuracy
...
f5_auc_test <- model_performance(ex_f5_test)$measures$auc
f5_acc_test <- model_performance(ex_f5_test)$measures$accuracy
...
```{r, echo=FALSE}
Model = factor(c("Logistic Regression","Random Forest", "Neural Network", "F5"))
Training_Accuracy = c(log_acc,rf_acc,nn_acc,f5_acc)  %>% round(digits = 3)
AUC = c(log_auc,rf_auc,nn_auc,f5_auc)  %>% round(digits = 3)
...
Test_Accuracy = c(log_acc_test,rf_acc_test,nn_acc_test,f5_acc_test)  %>% round(digits = 3)
AUC = c(log_auc_test,rf_auc_test,nn_auc_test,f5_auc_test)  %>% round(digits = 3)
...
RF over-fits training data so need to test them. -- https://mljar.com/blog/random-forest-overfitting/#:~:text=The%20Random%20Forest%20algorithm%20does,are%20added%20to%20the%20algorithm.&text=To%20avoid%20overfitting%20in%20Random,of%20samples%20in%20the%20leaf.
