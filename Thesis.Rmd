...
Following investigation of all training accuracy and AUC measures, it is obvious the random forest model outperformed all other models (figure \ref{tbl:data1}). The random forest model produced AUC of 1 and an accuracy of 0.994. This model shows great indication of a power model in predicting hypertension. Using the training data, the logistic regression and neural network models have yielded almost identical measures of accuracy and AUC.\ 
...
The test data was utilised to evaluate the fit of all models and to examine all measures of accuracy. All models performed relatively uniformly. Table \ref{tbl:data2} presents the model accuracy and AUC values. It is clear that all models produced very similar measures of both accuracy and AUC values. The logistic regression model performed marginally better than the neural network model by a decimal of 0.01 (AUC = 0.743 and accuracy = 0.806) and is the most accurate model in predicting hypertension. Both the logistic regression and neural network models have outperformed the random forest model. It is clear the random forest model over-fit the training data set prior to evaluating the model fit. 
...
```
 ...
 Figure \ref{fig:hist5} shows the ROC curve of all models using the test data set. The ROC curvle was generated by plotting all model performances and setting geom to "roc". From the plot it is clear how similar all model fits are. The logistic regression line is difficult to inspect as it is masked by the neural network line, although both performances were rather consistent. 
