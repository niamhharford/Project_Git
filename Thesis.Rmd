...
I, Niamh Harford declare that this thesis is my own work, except where stated through references or in the Acknowledgments and that it is 13,702 words in length.
...
To explore both the training and test data, data visualisation techniques were used to examine whether any patterns or relationships were present among the variables. Each explanatory variable was plotted with the response variable to assess if such patterns existed. After exploring the plots, several issues were identified. Firstly, it was apparent there were a considerable amount of larger values present in Potassium for both the training and test data (Figure \ref{fig:pot}). Potassium had very few extremely large values, with the majority of values within this variable being considerably small in a range of roughly 0 and 5000. Potassium was log transformed for both training and test data sets to reduce the skewness of the data. 
...
Furthermore, it was clear both data sets were comprised of a considerable number of missing values (Figure \ref{fig:hist0}). The training data had total of 1712 missing values, which were present across 903 rows of the data. The test data had a total of 1453 missing values, which were present across 754 rows of the data. These quantities of missing values were a substantial proportion of both the training data (0.322) and test data (0.265). Several options were explored to deal with missing values. These options included omitting rows of data that contained missing values or imputing the missing values. It was evident three variables in particular stood out as having largest quantities of missing values. These variables were alcohol, potassium and salt. 
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=9.7, fig.cap="Bar charts displaying the effect of salt use and diabetes on hypertension for the training data, prior to data cleaning.\\label{fig:b4clean2}",fig.show='hold'}
z <- ggplot(data_11) +
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3,fig.width=9.7,fig.align='default',fig.cap="Bar charts displaying the effect of salt use and diabetes on hypertension for the training data, following data cleaning.\\label{fig:after2}",fig.show='hold',fig.pos='!H'}
z <- ggplot(data_11a) +
      geom_bar(aes(x = Salt, fill = Hypertension)) + labs(x="Salt",fill="Hypertension") + scale_fill_manual(values=cbPalette[c(3,4,5)])
...
A neural network model is called a network as they can be associated with a directed acyclic graph describing how the functions are composed together. Neural networks contain an input layer, one or more hidden layers, and an output layer. More than often these models are more flexible than the logistic regression models (López-Martínez et al., 2020). Each node produced by neural network model is known as a unit. Each unit connects to another and has an associated weight and threshold. A neuron unit first takes a linear combination of the explanatory variables and applies a non-linear function $f$, called an activation function. The neural network model was fit with 'tidymodels', using the mlp() function with the mode set to "classification". The engine was set to "nnet" (Venables and Ripley, 2002). The model fit examined the relationship between the response variable and all explanatory variables.


It was necessary to further tune the neural network model using 'tidymodels', as its fit was not satisfactory. The model fit was updated specifying tune() for the hidden_units, penalty and epochs. A grid was created using the grid_regular() function and the range for tuning each argument was specified (Appendix 1). A range of 1 to 20 was defined for the hidden_units, 0.5 to 0.0000001 for the penalty and 10 to 10000 for the epochs. The final model was chosen based off the best "roc_auc" value. The final neural network model contained 20 hidden units, a penalty of	1.77828 and 5005 epochs.
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot assessing the average prediction of hypertension with the variables Gender and Smoking in the logistic regression model.\\label{fig:log_gen_smoke}",fig.show='hold'}
...
Figure \ref{fig:log_dia_kid} reveals the average prediction of hypertension for both Diabetes and KidneyF in the logistic regression model. The average prediction of hypertension revealed to be considerably higher for both individuals who have been diagnosed with diabetes and were borderline diagnosed with diabetes. Similarly, the average prediction of hypertension was seen to be noticeably higher for individuals who were diagnosed with kidney disease or failing kidneys.

\pagebreak

```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot assessing the average prediction of hypertension with the variables Diabetes and KidneyF in the logistic regression model.\\label{fig:log_dia_kid}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot assessing the average prediction of hypertension with the variables Alcohol and PhysActivity in the logistic regression model.\\label{fig:log_alco_pact}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
...
Figure \ref{fig:log_salt} shows the average prediction of hypertension for Salt in the logistic regression model. The plot illustrates the average prediction of hypertension is very similar across all levels of Salt. Occasionally and never using salt appear to have the same average prediction. There is evidently no obvious differences between the levels of salt used on food.

\pagebreak

```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot assessing the average prediction of hypertension with the variable Salt in the logistic regression model.\\label{fig:log_salt}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
                        variables = c("Salt"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=7.3,fig.cap="PD plot assessing the average prediction of hypertension with the variable Race in the logistic regression model.\\label{fig:log_race}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
                        variables = c("Race"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Gender and Smoking in the random forest model.\\label{fig:rf_gen_smok}",fig.show='hold'}
...
Figure \ref{fig:rf_dia_kid} illustrates the average prediction of hypertension for Diabetes and KidneyF in the random forest model. This plot revealed the average prediction of hypertension to be reasonably higher for individuals who have been diagnosed with diabetes and who were borderline diagnosed with diabetes. Similarly, the average prediction of hypertension was seen to be evidently higher for individuals who were diagnosed with kidney disease or failing kidneys. These results are comparable to the logistic regression model.

\pagebreak

```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Diabetes and KindeyF in the random forest model.\\label{fig:rf_dia_kid}",fig.show='hold'}
pdp_rf <- model_profile(ex_rf, N = NULL, 
                        variables = c("Diabetes","KidneyF"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Alcohol and PhysActivity in the random forest model.\\label{fig:rf_alco_pact}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variable Salt in the random forest model.\\label{fig:rf_salt}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=7.3,fig.cap="PD plot average assessing the prediction of hypertension with the variable Race in the random forest model..\\label{fig:rf_race}",fig.show='hold'}
pdp_rf <- model_profile(ex_rf, N = NULL, 
                        variables = c("Race"))
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.height=2.5,  fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Age, BMI and Potassium in the neural network model, assessing males and females separately.\\label{fig:hist4}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Gender and Smoking in the neural network model.\\label{fig:nn_gen_smo}",fig.show='hold'}
...
Figure \ref{fig:nn_dia_kid} displays the average prediction of hypertension for Diabetes and KidneyF in the neural network model. Further to the previous models, the neural network model estimated very similar average predictions of hypertension for both Diabetes and KidneyF. The average prediction of hypertension was estimated to be somewhat higher for individuals who have been diagnosed with diabetes and were borderline diagnosed with diabetes. Similarly, the average prediction of hypertension was higher for individuals who were diagnosed with kidney disease or failing kidneys.




```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Diabetes and KidneyF in the neural network model.\\label{fig:nn_dia_kid}",fig.show='hold'}
pdp_nn <- model_profile(ex_nn, N = NULL, 
                        variables = c("Diabetes","KidneyF"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Alcohol and PhysActivity in the neural network model.\\label{fig:nn_alco_pact}",fig.show='hold'}
pdp_nn <- model_profile(ex_nn, N = NULL, 
                        variables = c("PhysActivity", "Alcohol"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=5,fig.cap="PD plot average assessing the prediction of hypertension with the variables Salt in the neural network model.\\label{fig:nn_salt}",fig.show='hold'}
...
Figure \ref{fig:nn_race} shows the average prediction of hypertension for Race in the neural network model. The average prediction of hypertension revealed non-Hispanic black individuals have the highest average prediction of hypertension in contrast to the other ethnic groups. However, the average prediction of hypertension for all ethnic groups appeared to be higher to a degree for the neural network model compared to the previous models. In particular, the ethnic groups disregarding non-Hispanic black individuals, have had moderately higher average predictions of hypertension in this model and it is apparent there are no major differences between these groups on the prediction of hypertension.

\pagebreak

```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=7.3,fig.cap="PD plot average assessing the prediction of hypertension with the variables Race in the neural network model.\\label{fig:nn_race}",fig.show='hold'}
pdp_nn <- model_profile(ex_nn, N = NULL, 
                        variables = c("Race"))
...
Table \ref{tbl:data2_tr} presents the model performances calculated using the training data. It is immediately obvious the random forest model outperformed the other models by examining the metrics produced. The random forest model produced an almost perfect AUC value of 0.99, in addition to all other measures being extremely high indicating towards a very accurate model. The logistic regression and neural network models in particular performed identically, by producing the same result for all measures assessed.
...
Further examining the measures produced from the confusion matrix, it was evident that all models performed quite similarly when evaluated with the test data. Table \ref{tbl:data2} presents the model performances with measures calculated using the test data. The logistic regression and neural network models in particular produced very similar measures of both accuracy and AUC values. Three out of five of the measures displayed in table \ref{tbl:data2} demonstrate how the neural network model and the logistic regression model performed identically, as they both produced the same value for the TPR, TNR and accuracy. The logistic regression model produced an AUC value 0.01 higher the AUC value produced by the neural network model, however this difference is marginal. The neural network model yielded the highest precision value. 
...
By analysing the prediction of hypertension with three alternative machine learning models utilising NHANES data, this thesis has shown that both neural network and logistic regression models are the most accurate at predicting hypertension. The logistic regression model is the most practical model for medical researchers to make meaningful inferences on hypertension for the future prevention and control of this condition. This analysis has further supported that there are multiple reliable and adequate models that can assist in the prediction of conditions and diseases. This type of analysis can be extremely beneficial to humans all around the world. Machine learning prediction-based models have the capability to prevent some of these conditions like hypertension from transpiring and further preventing extensive cardiovascular deaths globally. 
...
3. Sakr, S., Elshawi, R., Ahmed, A., Qureshi, W., Brawner, C., Keteyian, S., Blaha, M. and Al-Mallah, M., 2018. Using machine learning on cardiorespiratory fitness data for predicting hypertension: The Henry Ford ExercIse Testing (FIT) Project. *PLOS ONE,* 13(4), pp.e0195344.
...
11. Hamrahian, S.M. and Falkner, B., 2016. *Hypertension in chronic kidney disease. Hypertension: from basic research to clinical practice*, pp.307-325.
12. Aneja, A., El-Atat, F., McFarlane, S.I. and Sowers, J.R., 2004. Hypertension and obesity. *Recent progress in hormone research*, 59(1), pp.169-205.
13. Prolo, P., Wong, M.L. and Licinio, J., 1998. Leptin. *The international journal of biochemistry & cell biology*, 30(12), pp.1285-1290.
14. Centers for Disease Control and Prevention. 2021. The Role of Potassium and Sodium in Your Diet. [online] Available at: <https://www.cdc.gov/salt/potassium.htm> [Accessed 13 July 2021].
15. Ekmekcioglu, C., Elmadfa, I., Meyer, A.L. and Moeslinger, T., 2016. The role of dietary potassium in hypertension and diabetes. *Journal of physiology and biochemistry*, 72(1), pp.93-106.
16. Kourou, K., Exarchos, T.P., Exarchos, K.P., Karamouzis, M.V. and Fotiadis, D.I., 2015. Machine learning applications in cancer prognosis and prediction. *Computational and structural biotechnology journal*, 13, pp.8-17.
17. Kononenko, I., 2001. Machine learning for medical diagnosis: history, state of the art and perspective. *Artificial Intelligence in medicine*, 23(1), pp.89-109.
...
19. López-Martínez, F., Schwarcz, A., Núñez-Valdez, E.R. and Garcia-Diaz, V., 2018. Machine learning classification analysis for a hypertensive population as a function of several risk factors. *Expert Systems with Applications*, 110, pp.206-215.
20. cdc.gov. 2021. About the NHANES. [online] Available at: <https://www.cdc.gov/nchs/nhanes/about_nhanes.htm> [Accessed 31 May 2021].
21. Christopher J. Endres (2021). nhanesA: NHANES Data Retrieval. R package version 0.6.5.3. https://CRAN.R-project.org/package=nhanesA
22. Kuhn et al., (2020). Tidymodels: a collection of packages for modeling and machine learning using tidyverse principles. https://www.tidymodels.org
23. Hall, G.H. and Round, A.P., 1994. Logistic regression—explanation and use. *Journal of the Royal College of Physicians of London*, 28(3), pp.242.
24. Breiman, L., 2001. Random forests. *Machine learning*, 45(1), pp.5-32.
25. Wright, M.N. and Ziegler, A., 2017. ranger: A Fast Implementation of Random Forests for High Dimensional Data in C++ and R. *Journal of Statistical Software*, 77(1),pp.1-17. doi:10.18637/jss.v077.i01
26. Prasad, A.M., Iverson, L.R. and Liaw, A., 2006. Newer classification and regression tree techniques: bagging and random forests for ecological prediction. *Ecosystems*, 9(2), pp.181-199.
27. Venables, W. N. & Ripley, B. D., 2002. Modern Applied Statistics with S. Fourth Edition. Springer, New York. ISBN 0-387-95457-0
28. Friedman, J.H., 2001. Greedy function approximation: a gradient boosting machine. *Annals of statistics*, pp.1189-1232.
29. Swets, J.A., 1979. ROC analysis applied to the evaluation of medical imaging techniques. *Investigative radiology*, 14(2), pp.109-121.
30. Biecek P., 2018. “DALEX: Explainers for Complex Predictive Models in R.” _Journal of Machine Learning Research_, 19(84), 1-5. <URL: https://jmlr.org/papers/v19/18-416.html>.
31. Biecek, P. and Burzykowski, T., 2021. *Explanatory model analysis: explore, explain, and examine predictive models*. CRC Press.
32. Goldstein, A., Kapelner, A., Bleich, J. and Pitkin, E., 2015. Peeking inside the black box: Visualizing statistical learning with plots of individual conditional expectation. *Journal of Computational and Graphical Statistics*, 24(1), pp.44-65.
33. Maksymiuk S., Gosiewska A. and Biecek P., 2020. “Landscape of R packages for eXplainable Artificial Intelligence.” _arXiv_. Pages 6, 7, 11, 15, <URL: https://arxiv.org/abs/2009.13248>.
34. Dreiseitl, S. and Ohno-Machado, L., 2002. Logistic regression and artificial neural network classification models: a methodology review. *Journal of biomedical informatics*, 35(5-6), pp.352-359.
35. Kayame, R. and Mallongi, A., 2018. Relationships between Smoking Habits and the Hypertension Occurrence among the Adults of Communities in Paniai Regency, Papua Indonesia. *Indian Journal of Public Health Research & Development*, 9(1).
36. Suter, P.M., 1998. Potassium and hypertension. *Nutrition reviews*, 56(5), pp.151-153.
37. Global Health Estimates, 2014. *Summary Tables Deaths by Cause, Age and Sex*, 2000–2012. World Health Organization.
38. Cso.ie. 2012. Vital Statistics Fourth Quarter and Yearly Summary 2011. [online] Available at: <https://www.cso.ie/en/csolatestnews/pressreleases/2012pressreleases/pressreleasevitalstatisticsfourthquarterandyearlysummary2011/> [Accessed 12 July 2021].
39. Bennett, K., Kabir, Z., Unal, B., Shelley, E., Critchley, J., Perry, I., Feely, J. and Capewell, S., 2006. Explaining the recent decrease in coronary heart disease mortality rates in Ireland, 1985–2000. *Journal of Epidemiology & Community Health*, 60(4), pp.322-327.
40. Zhao, Y., Yan, H., Marshall, R.J., Dang, S., Yang, R., Li, Q. and Qin, X., 2013. Trends in population blood pressure and prevalence, awareness, treatment, and control of hypertension among middle-aged and older adults in a rural area of Northwest China from 1982 to 2010. *PloS one*, 8(4), pp.e61779.
41. Kearney, P.M., Whelton, M., Reynolds, K., Muntner, P., Whelton, P.K. and He, J., 2005. Global burden of hypertension: analysis of worldwide data. *The Lancet*, 365(9455), pp.217-223.
...
**Appendix 1. ** Code for tuning the neural network model using tidymodels.

```{r, echo=TRUE,eval=FALSE}
nn_mod <- mlp(mode = "classification") %>% set_engine("nnet")

nn_fit <-nn_mod %>% fit(Hypertension ~ ., data=data_11a)

nnet_mod1 <-nn_mod %>% update(hidden_units = tune(), penalty=tune(), epochs=tune())

nnet_grid <- grid_regular(hidden_units(range=c(1,20)),
  penalty(range =c(0.5,0.0000001)), epochs(range= c(10,10000)))

folds <- vfold_cv(data_11a, v = 10)

nn_wf <- workflow() %>% add_model(nnet_mod1) %>% add_formula(Hypertension ~ .)
...
best_nn <- nn_res %>% select_best("roc_auc")

final_wf <- 
  nn_wf %>%  finalize_workflow(best_nn)

# final nn model
final_nn <- 
  final_wf %>% fit(data = data_11a) 
```
