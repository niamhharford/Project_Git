...
Training_Accuracy = c(tm_log_acc,tm_rf_acc,tm_nn_acc)  %>% round(digits = 3)
AUC = c(tm_log_auc,tm_rf_auc,tm_nn_auc)  %>% round(digits = 3)
...
All 3 models produced very similar average predictions of hypertension for all explanatory variables. The logistic regression model reveals an estimated sensitivity of `r sensitivity_lr %>% round(digits = 2) *100`%, which are values of hypertension correctly positively classified as 1 or "Yes". This models displays an estimated specificity of `r specificity_lr %>% round(digits = 2) *100`%, which are values of hypertension correctly negatively classified as 0 or "No". The logistic regression model produced an estimated positive predicted value of `r precision_lr %>% round(digits = 2) *100`%, and a negative predictive value of `r neg_pred_lr %>% round(digits = 2) *100`%. The model produced an estimated false negative rate of `r type2_error_lr %>% round(digits = 2) *100`%, and a false positive rate of `r type2_error_lr %>% round(digits = 2) *100`%.

Following investigation of both training accuracy and AUC measures, it was clear the random forest model outperformed all other models (table \ref{tbl:data1}). The random forest model produced extremely high and satisfactory measures (AUC = `r tm_rf_acc %>% round(digits = 3)` and accuracy = `r tm_rf_auc %>% round(digits = 3)`). This model shows great indication of a powerful model in predicting hypertension. From the training data, the logistic regression and neural network models yielded almost identical measures of accuracy and AUC.\ 




With the application of the test data to evaluate the model fits, all three confusion matrices were produced to investigate the summary of predicted results for all models (figures \ref{fig:cf1}, \ref{fig:cf2}, \ref{fig:cf3}). 
...
Test_Accuracy = c(tm_log_acc_test, tm_rf_acc_test,tm_nn_acc_test)  %>% round(digits = 3)
AUC = c(tm_log_auc_test, tm_rf_auc_test,tm_nn_auc_test)  %>% round(digits = 3)
...
Figure \ref{fig:roc} shows the ROC curve of all models using the test data set. The ROC curve was generated by plotting all model performances and setting geom to "roc". From the plot it is clear how similar all model fits are. The logistic regression line is difficult to inspect as it is masked by the neural network line, although both performances were rather consistent. 
