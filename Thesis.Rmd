...
I, Niamh Harford declare that this thesis is my own work, except where stated through references or in the Acknowledgements and that it is ? words in length.
...
Figure \ref{fig:rf_salt} shows the average prediction of Salt for the random forest model. The plot illustrates the average prediction of hypertension is very similar across three levels of Salt. Never using salt has the highest average prediction of hypertension, albeit the difference is negligible.
...
Figure \ref{fig:nn_salt} shows the average prediction of Salt for the neural network model. The plot illustrates the average prediction of hypertension is almost identical for all levels of Salt. There is no major variance among the average predictions of hypertension for Salt.
...
```{r, echo=FALSE,fig.height=2}
ggplot(data_11) +
  geom_bar(aes(x = PhysActivity, fill = Hypertension)) + labs(x="Physical Activity", y = "Hypertension") 
...
```{r, echo=FALSE,fig.height=2}
ggplot(data_11) +
  geom_bar(aes(x = Gender, fill = Hypertension)) + labs(x="Gender") 
```
...
```{r,echo=FALSE, fig.height=2}
ggplot(data_11) +
  geom_bar(aes(x = Race, fill = Hypertension)) + labs(x="Gender") 
```