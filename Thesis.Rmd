...
ex_log <- DALEX::explain(glm_fit, data = data_11a, y = hypn, type = "classification", label = "Random Forest")
ex_rf <- DALEX::explain(rf_fit, data = data_11a, y = hypn, type = "classification", label = "Logistic Regression")
ex_nn <-  DALEXtra::explain_tidymodels(final_nn, data = data_11a, y = hypn, type = "classification", label = "Neural Network")
...
ex_log_test <- DALEX::explain(glm_fit, data = Test, y = hypn1, type = "classification", label = "Random Forest")
ex_rf_test <- DALEX::explain(rf_fit, data = Test, y = hypn1, type = "classification", label = "Logistic Regression")
ex_nn_test <-  DALEX::explain(final_nn, data = Test, y = hypn1, type = "classification", label = "Neural Network")
...
plot_model(glm_fit, vline.color = "black", sort.est = TRUE,colors = "Accent")
```
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.cap="Partial dependence plot on the test data for the logistic regression response for age, bmi and potassium, for males and females separately.\\label{fig:hist2b}",fig.show='hold'}
plot(model_profile(ex_log_test,groups="Gender")) + ggtitle("Response for Logistic Regression") + theme(plot.title = element_text(size = 10))
```
...
```{r, eval=FALSE, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.height=4, fig.cap="Variable importance for the random forest model.\\label{fig:rf_var}",fig.show='hold'}
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.height=3, fig.cap="Partial dependence plot on the test data for the neural network response for age, bmi and potassium, for males and females separately.\\label{fig:hist4b}",fig.show='hold'}
plot(model_profile(ex_nn_test,groups="Gender")) + ggtitle("Response for Neural Network")+ theme(plot.title = element_text(size = 10))
```
