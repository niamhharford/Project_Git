...
plot_model(glm_fit, vline.color = "black", sort.est = TRUE,colors = "Accent", title = "") + labs(x = "Explanatory Variables")
```
...
From further examination of the  measures produced from the evaluation of the test data, it was evident that all models performed quite similarly. Table \ref{tbl:data2} presents the model performances with measures including accuracy and AUC values. The logistic regression and neural network models in particular produced very similar measures of both accuracy and AUC values. For 3 out of 5 of the measures displayed in \ref{tbl:data2}, the neural network model and the logistic regression model performed identically. They both produced the same value for the TPR, TNR and accuracy. The logistic regression model produced an AUC value 0.01 higher the AUC value produced by the neural network model, however this difference is marginal. The neural network model yielded the highest precision value. It should be noted the random forest model did not perform as satisfactorily as the other models. 
...
Several studies have taken multiple approaches to predict hypertension by means of utilising alternative models and additionally different data. This project took a machine learning approach for predicting hypertension using data from NHANES. Using the NHANES data, the analysis focused on assessing 3 alternative models. The models implemented were logistic regression, random forest and neural network. Each model was examined to determine the most appropriate model in predicting hypertension. Additionally, each variable was investigated to establish potential leading causes of hypertension and to establish the most important variables when predicting hypertension. The findings are considered in 4.1. Model Selection and 4.2. Variable Importance.
