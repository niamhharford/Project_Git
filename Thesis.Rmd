...
ggplot(data_11) +
      geom_boxplot(aes(x = Potassium, y = Hypertension)) + labs(x="Potassium", y ="Hypertension") + coord_flip()

```


Furthermore, it was clear both data sets were comprised of a considerable number of missing values (Figure \ref{fig:hist0}). The training data had total of 1712 missing values, which were present across 903 rows of the data. The test data had a total of 1453 missing values, which were present across 754 rows of the data. This quantity of missing values was a substantial proportion of both the training data (0.322) and test data (0.265). Several options were explored to deal with missing values. These options included omiting rows of data that contained missing values or imputing the missing values. It was evident three variables in particular stood out as having the large amounts of missing values. These variables were alcohol, potassium and salt. 



```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3.7, fig.cap="Missing values\\label{fig:hist0}"}
...
It was necessary to further tune the neural network model using 'tidymodels', as its fit was not satisfactory. The model fit was updated specifying tune() for the hidden_units, penalty and epochs. A grid was created using the grid_regular() function and the range for tuning each argument was specified. A range from 0 to 20 was defined for the hidden_units, 0.5 to 0.0000001 for the penalty and 10 to 10000 for the epochs. The best model was chosen based of the best "roc_auc" value. The final neural network model contained 20 hidden units, a penalty of	1.77828 and 5005 epochs.
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.height=3.4, fig.width=5,fig.cap="The odds ratios for the logistic regression.\\label{fig:hist1}",fig.show='hold'}
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5, fig.width=5,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Age, BMI and Potassium with males and females separately.\\label{fig:hist2}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Gender and Smoking.\\label{fig:log_gen_smoke}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Diabetes and KidneyF.\\label{fig:log_dia_kid}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Alcohol and PhysActivity.\\label{fig:log_alco_pact}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Race.\\label{fig:log_salt}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
                        variables = c("Salt"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Race.\\label{fig:log_race}",fig.show='hold'}
pdp_log <- model_profile(ex_log, N = NULL, 
                        variables = c("Race"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the random forest model, for Gender and Smoking.\\label{fig:rf_gen_smok}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the random forest model, for Diabetes and KidneyF.\\label{fig:rf_dia_kid}",fig.show='hold'}
pdp_rf <- model_profile(ex_rf, N = NULL, 
                        variables = c("Diabetes","KidneyF"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the random forest model, for Alcohol and PhysActivity.\\label{fig:rf_alco_pact}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Race.\\label{fig:rf_salt}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the random forest model, for Race.\\label{fig:rf_race}",fig.show='hold'}
pdp_rf <- model_profile(ex_rf, N = NULL, 
                        variables = c("Race"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.height=2.3, fig.width=5, fig.cap="Variable importance on the training data for the random forest model.\\label{fig:rf_var}",fig.show='hold'}
suppressMessages(library(vip))
rf_fit %>%
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the neural network model, for Gender and Smoking.\\label{fig:nn_gen_smo}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the neural network model, for Diabetes and KidneyF.\\label{fig:nn_dia_kid}",fig.show='hold'}
pdp_nn <- model_profile(ex_nn, N = NULL, 
                        variables = c("Diabetes","KidneyF"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the neural network model, for Alcohol and PhysActivity.\\label{fig:nn_alco_pact}",fig.show='hold'}
pdp_nn <- model_profile(ex_nn, N = NULL, 
                        variables = c("PhysActivity", "Alcohol"))
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the logistic regression model, for Race.\\label{fig:nn_salt}",fig.show='hold'}
...
```{r,include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=2.5,fig.width=4,fig.cap="Partial dependence plot on the training data for the neural network model, for Race.\\label{fig:nn_race}",fig.show='hold'}
pdp_nn <- model_profile(ex_nn, N = NULL, 
                        variables = c("Race"))
...
Table \ref{tbl:data2} presents the model performances using the training data. It is immediately obvious the random forest model outperformed the other models using the metrics produced by the training data. The random forest model produced an almost perfect AUC value at 0.99, in addition to all other measures being extremely high indicating towards a very accurate model. The logistic regression and neural network models in particular performed identically, by producing the same result for all measures assessed.
...
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.height=4, fig.cap="Test ROC curve for all models using the test data.\\label{fig:roc}",fig.show='hold'}
...
10. Ekmekcioglu, C., Elmadfa, I., Meyer, A.L. and Moeslinger, T., 2016. The role of dietary potassium in hypertension and diabetes. *Journal of physiology and biochemistry*, 72(1), pp.93-106.
11. Kourou, K., Exarchos, T.P., Exarchos, K.P., Karamouzis, M.V. and Fotiadis, D.I., 2015. Machine learning applications in cancer prognosis and prediction. *Computational and structural biotechnology journal*, 13, pp.8-17.
12. Kononenko, I., 2001. Machine learning for medical diagnosis: history, state of the art and perspective. *Artificial Intelligence in medicine*, 23(1), pp.89-109.
...
14. López-Martínez, F., Schwarcz, A., Núñez-Valdez, E.R. and Garcia-Diaz, V., 2018. Machine learning classification analysis for a hypertensive population as a function of several risk factors. *Expert Systems with Applications*, 110, pp.206-215.
15. cdc.gov. 2021. About the NHANES. [online] Available at: <https://www.cdc.gov/nchs/nhanes/about_nhanes.htm> [Accessed 31 May 2021].
16. Christopher J. Endres (2021). nhanesA: NHANES Data Retrieval. R package version 0.6.5.3. https://CRAN.R-project.org/package=nhanesA
...
18. Hall, G.H. and Round, A.P., 1994. Logistic regression—explanation and use. Journal of the Royal College of Physicians of London, 28(3), p.242.
19. Breiman, L., 2001. Random forests. *Machine learning*, 45(1), pp.5-32.
20. Wright, M.N. and Ziegler, A., 2017. ranger: A Fast Implementation of Random Forests for High Dimensional Data in C++ and R. Journal of Statistical Software, 77(1), 1-17. doi:10.18637/jss.v077.i01
21. Prasad, A.M., Iverson, L.R. and Liaw, A., 2006. Newer classification and regression tree techniques: bagging and random forests for ecological prediction. *Ecosystems*, 9(2), pp.181-199.
22. Venables, W. N. & Ripley, B. D. (2002) Modern Applied Statistics with S. Fourth Edition. Springer, New York. ISBN 0-387-95457-0
23. Friedman, J.H., 2001. Greedy function approximation: a gradient boosting machine. *Annals of statistics*, pp.1189-1232.
24. Swets, J.A., 1979. ROC analysis applied to the evaluation of medical imaging techniques. *Investigative radiology*, 14(2), pp.109-121.
25. Biecek P (2018). “DALEX: Explainers for Complex Predictive Models in R.” _Journal of Machine Learning Research_, 19(84), 1-5. <URL: https://jmlr.org/papers/v19/18-416.html>.
26. Biecek, P. and Burzykowski, T., 2021. *Explanatory model analysis: explore, explain, and examine predictive models*. CRC Press.
27. Goldstein, A., Kapelner, A., Bleich, J. and Pitkin, E., 2015. Peeking inside the black box: Visualizing statistical learning with plots of individual conditional expectation. *Journal of Computational and Graphical Statistics*, 24(1), pp.44-65.
28. Maksymiuk S, Gosiewska A, Biecek P (2020). “Landscape of R packages for eXplainable Artificial Intelligence.” _arXiv_. Pages 6, 7, 11, 15, <URL: https://arxiv.org/abs/2009.13248>.
29. Dreiseitl, S. and Ohno-Machado, L., 2002. Logistic regression and artificial neural network classification models: a methodology review. *Journal of biomedical informatics*, 35(5-6), pp.352-359.
30. Kayame, R. and Mallongi, A., 2018. Relationships between Smoking Habits and the Hypertension Occurrence among the Adults of Communities in Paniai Regency, Papua Indonesia. *Indian Journal of Public Health Research & Development*, 9(1).
31. Suter, P.M., 1998. Potassium and hypertension. *Nutrition reviews*, 56(5), pp.151-153.
32. Global Health Estimates, 2014. *Summary Tables Deaths by Cause, Age and Sex*, 2000–2012. World Health Organization.
33. Cso.ie. 2012. Vital Statistics Fourth Quarter and Yearly Summary 2011. [online] Available at: <https://www.cso.ie/en/csolatestnews/pressreleases/2012pressreleases/pressreleasevitalstatisticsfourthquarterandyearlysummary2011/> [Accessed 12 July 2021].
34. Bennett, K., Kabir, Z., Unal, B., Shelley, E., Critchley, J., Perry, I., Feely, J. and Capewell, S., 2006. Explaining the recent decrease in coronary heart disease mortality rates in Ireland, 1985–2000.* Journal of Epidemiology & Community Health*, 60(4), pp.322-327.
35. Zhao, Y., Yan, H., Marshall, R.J., Dang, S., Yang, R., Li, Q. and Qin, X., 2013. Trends in population blood pressure and prevalence, awareness, treatment, and control of hypertension among middle-aged and older adults in a rural area of Northwest China from 1982 to 2010. *PloS one*, 8(4), pp.e61779.
36. Kearney, P.M., Whelton, M., Reynolds, K., Muntner, P., Whelton, P.K. and He, J., 2005. Global burden of hypertension: analysis of worldwide data. *The Lancet*, 365(9455), pp.217-223.
