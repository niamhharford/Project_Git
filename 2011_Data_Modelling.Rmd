...
log_reg1 <- glm(Hypertension ~ . + I(Age^2) + I(Age^3), data = data_11a, family = binomial())
summary(log_reg1)
```
...
+ Occasionally Using Salt in Food Prep
...
gam_logistic = gam(Hypertension ~ Gender + bs(Age, df = 5) + Race + Smoking + Diabetes + 
                    KidneyF + BMI + PhysActivity + Alcohol + Salt +
...
anova(gam,gam_logistic) # p-value > 0.05 so reject null, the b spline on Age is appropriate
...
data_11a$Hypertension <-factor(ifelse(data_11a$Hypertension == "Yes", 1 ,0))
data_11a$Hypertension %>% table
log_reg_gender <- glm(Hypertension ~ (. + I(Age^2) + I(Age^3))* Gender, data = data_11a, family = binomial())
...
male <- plot_model(log_reg_gender, sort.est = TRUE, vline.color = "black", colors = cbPalette[c(3,4)], terms = c("Age","Race [Other Hispanic]","Race [Non-Hispanic White]", "Race [Non-Hispanic Black]", "Race [Other Race - Including Multi-Rac]", "Smoking [Yes]", "Diabetes [Yes]", "Diabetes [Borderline]", "KidneyF [Yes]", "BMI", "PhysActivity [Yes]", "Alcohol [Yes]", "Salt [Rarely]", "Salt [Occasionally]", "Salt [Very Often]", "Potassium", "Age^3", "Age^2"), title = "Male Odds of Hypertension") 

female <- plot_model(log_reg_gender, sort.est = TRUE, vline.color = "black", colors = cbPalette[c(3,4)], rm.terms = c("Age","Race [Other Hispanic]","Race [Non-Hispanic White]", "Race [Non-Hispanic Black]", "Race [Other Race - Including Multi-Rac]", "Smoking [Yes]", "Diabetes [Yes]", "Diabetes [Borderline]", "KidneyF [Yes]", "BMI", "PhysActivity [Yes]", "Alcohol [Yes]", "Salt [Rarely]", "Salt [Occasionally]", "Salt [Very Often]", "Potassium", "Age^3", "Age^2"), title = "Female Odds of Hypertension")
...
```{r}
# GAM
model_gam_logistic = gam(Hypertension ~ Gender + bs(Age, df = 5) + Race + Smoking + Diabetes + KidneyF + BMI + PhysActivity + Alcohol + Salt +
                     Potassium, data=data_11a, family = binomial())
```
...
plot(sv_rf, sv_nn, sv_log1, sv_log2)
...
grid.arrange(p1, p2, p3,p4,  nrow = 1, top ="Model Comparison")
```
