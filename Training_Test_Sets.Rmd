...
# Training error
preda <- predict(f2, Train)$class
mean(preda != Train$Hypertension)*100 # training error

# Test error
predb <- predict(f2, Test)$class
...
# Training Data
pred <- predict(f3, Train, type="class")
tab2 <- table(pred, Train$Hypertension)
tab2


# Nurea Network training Error rate
(tab2[1]+tab2[4]) / sum(tab2) *100 # around 25-27%



# Test Data
pred <- predict(f3, Test,type="class")
tab3 <- table(pred, Test$Hypertension)
tab3


# Neural Network test Error rate
(tab3[1]+tab3[4]) / sum(tab3) *100 # ~26-27%
